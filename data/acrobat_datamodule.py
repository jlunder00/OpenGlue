...
from data.acrobat_dataset import AcrobatPairsDataset, AcrobatPairsDatasetFeatures
from data.acrobat_balanced_sampler import AcrobatBalancedSampler


class BaseAcrobatPairsDataModule(pl.LightningDataModule):
    def __init__(self, root_path, train_list_path, val_list_path, test_list_path,
        ...
        super(BaseAcrobatPairsDataModule, self).__init__()
    ...
    def train_dataloader(self):
        sampler = AcrobatBalancedSampler(self.train_ds) if self.balanced_train else None
        return DataLoader(
            self.train_ds,
            sampler=sampler,
...
class AcrobatPairsDataModule(BaseAcrobatPairsDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.train_ds = AcrobatPairsDataset(
        ...
        self.val_ds = AcrobatPairsDataset(
            root_path=self.root_path,
...
class AcrobatPairsDataModuleFeatures(BaseAcrobatPairsDataModule):
    def __init__(self, root_path, train_list_path, val_list_path, test_list_path,
                 batch_size, num_workers, target_size, features_dir, num_keypoints, val_max_pairs_per_scene,
                 balanced_train=False, train_pairs_overlap=None):
        super(AcrobatPairsDataModuleFeatures, self).__init__(
            root_path, train_list_path, val_list_path, test_list_path,
            batch_size, num_workers, val_max_pairs_per_scene, balanced_train,
            train_pairs_overlap
        )
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.train_ds = AcrobatPairsDatasetFeatures(
            root_path=self.root_path,
        ...
        self.val_ds = AcrobatPairsDatasetFeatures(
            root_path=self.root_path,
            features_dir=self.features_dir,
            scenes_list=self.read_scenes_list(self.val_list_path),
            target_size=self.target_size,
